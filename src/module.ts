...
function checkDirection(missing: Direction[][], toCheck: Direction[]) {
    const available = filterModules(modules, toCheck);
...
export function check() {
    ...
    for (const dir1 of Direction.values) {
        checkDirection(missing, [dir1]);
        for (const dir2 of Direction.values) {
            if (dir2 != dir1) {
                checkDirection(missing, [dir1, dir2]);
                for (const dir3 of Direction.values) {
                    if (dir3 != dir2 && dir3 != dir1) {
                        checkDirection(missing, [dir1, dir2, dir3]);
                        for (const dir4 of Direction.values) {
...
export function create(_id: string, directions: (Direction | string)[], maze: (number | string)[][], enemies: { pos: [number, number], type: number }[], tiles: TileDefinition<any> = {}, goalPos: [number, number]) {
